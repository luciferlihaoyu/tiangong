...
- 自动生成/持久化路径（来源 2/3）**假定天宫单实例部署**：每个实例会各自生成一把
  Ed25519 密钥，多实例下 JWKS 端点返回的公钥随负载均衡漂移，接收端验签会间歇失败；
- 多实例 / cluster 部署**必须**给所有实例显式配置同一把 `SSO_SIGNING_KEY_JWK`；
- 同理，璇玑侧 jti 一次性账本是单实例内存 Map——多实例璇玑需要粘性会话或外置账本，
  该前提与 v1 相同，v2 未改变。
