...
- **外部认领无CAS**（未做，见 Phase A 收尾）：task-claim.ts:144-159先查后按id更新。单进程async也可能交错双认领。WHERE带预期状态/版本，检查真实affected rows，胜者才返回任务及触发副作用。注：本轮已就绪 `getAffectedRows`，正是该项所需的前置能力。
...
| 阶段 | 内容 | 依赖与验收 |
|---|---|---|
| A立即，约1–2周 | 权限、真实SQLite事务、返回值/幂等/CAS、outbox筛选、深链 | 权限/深链可独立；事务先于并发验收。非授权拒绝，中途失败回滚，重复请求同任务，双认领一成功，100历史条后新事件可发送 |
