...
- 🟡（本轮完成切片 1）**状态多维度一致投影 + 修订号**：三个维度（`status` 粗粒度 / `lifecycleStatus` 生命周期机 / `boardStatus` 任务板机）按路线图**不合成单枚举**，改由新建的 `api/lib/task-transition.ts` 做唯一入口——命名转移先过状态机校验（规则从 `api/a2a-router.ts` 逐字搬入，a2a 路由改为引用，不再两处各维护一套），然后**一次 UPDATE 写齐所有维度 + `stateRevision + 1` + `updatedAt`**，并由 `lifecycleStatus` 推导粗粒度 `status`（completed→done，失败族→failed）与终态时间戳；非终态转移不动 status、不盖终态戳。WHERE 里带修订号做 CAS（即使调用方不传 `expectedRevision` 也用读到的那一版），读后被人改过就返回 `revision_conflict` 而不是覆盖别人的写入。
  ...
  - **仍未接入**：`task-runner` 九处（内部执行主链路）。它目前**没有任何测试缝**——`TaskRunner` 的执行方法全是 `private`，测试里只 `import` 过模块、从未调用，即**这条最常走的路径目前没有覆盖其状态写入的测试**。改它等于"没有红过的改代码"，因此排为后续切片，须先补一个可测入口（最小 `runOnce()` 缝）再加执行链路测试。其余：`taskboard-router` 的 `approve`/`reject`/`requestChanges`/`submit`/`updateStatus`（含两处动态 `updateFields`）、`orchestration-router`、`taskboard-notify`、`task-validator`、`mailbox-router`、`task-writeback`、若干 sweeper（含 requeue 分支）、`dag-router`、`auto-approve`、`execution-gate`、`collaboration-events`。
