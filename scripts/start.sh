...
#   a) 未设置（推荐）→ 默认 /app/data/tiangong-artifacts/tiangong.db
#      （TIANGONG_ARTIFACT_ROOT 下，由既有 tiangong-artifacts volume 持久化）
#   b) 旧 MySQL DSN（mysql://…，迁移过渡期由 connection.ts 兜底到 artifact 卷）
#   c) 显式 SQLite 文件路径
if [ -z "$DATABASE_URL" ]; then
  echo "✅ DATABASE_URL: 未设置 — 使用默认 SQLite 路径 \${TIANGONG_ARTIFACT_ROOT}/tiangong.db"
elif printf '%s' "$DATABASE_URL" | grep -q '^mysql://\|^mysql2://'; then
  echo "⚠️  DATABASE_URL: MySQL DSN（${DATABASE_URL:0:30}...）— 迁移过渡期，将兜底到 \${TIANGONG_ARTIFACT_ROOT}/tiangong.db"
else
  echo "✅ DATABASE_URL: SQLite 文件路径 ${DATABASE_URL}（不存在则启动时创建）"
fi
echo ""
echo "   ⚠️  若使用 SQLite，请确认 zeabur.json 的 /app/data/tiangong-artifacts volume 挂载已生效（数据持久化）"
