// Type shim for better-sqlite3 — see ./package.json for rationale.
// Tiangong's S1 adapter (api/lib/node-sqlite-adapter.ts) returns a value
// that is type-asserted to `import('better-sqlite3').Database` at the
// single permitted call site in connection.ts. The shapes below mirror
// the upstream @types/better-sqlite3 surface that drizzle-orm/better-sqlite3
// actually consumes; nothing here is executed at runtime.
...
declare namespace Database {
  ...
  type Statement<BindParameters extends unknown[] | {} = unknown[], Result = unknown> =
    BindParameters extends unknown[] ? BetterSqlite3.Statement<BindParameters, Result> : BetterSqlite3.Statement<[BindParameters], Result>;
