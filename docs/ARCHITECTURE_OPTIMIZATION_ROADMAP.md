...
- db/schema.ts:94-119的status/lifecycleStatus/boardStatus是不同维度。以单一transition服务维护一致投影和修订号，不粗暴合成单枚举。**（生产实测已证实会不一致）**：§3-2 验证时在**生产库**上取消一个 `dispatched` 任务，结果 `status='failed'`、`error='[cancelled] …'`，而 `lifecycle_status` 仍是 `dispatched`、`failed_at` 仍是空——同一行上两个维度互相矛盾。原因是 MCP `cancel_task` 走的是「只改 status」的旧写法（本轮为**保留既有语义**刻意没动它），它既没推进生命周期投影也没盖终态时间戳；这正是需要单一 transition 服务的地方，而不是靠各调用点自觉补字段。
